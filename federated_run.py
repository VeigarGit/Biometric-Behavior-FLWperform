...
from sktime_dl.classification import InceptionTimeClassifier, CNNClassifier, ResNetClassifier, \
    MCDCNNClassifier, TapNetClassifier, MACNNClassifier, TLENETClassifier, FCNClassifier
...
parser.add_argument('--nn', type=str, default='FCN')
...
if args.nn == 'inception':
    ...
    network = TapNetClassifier()
elif args.nn =='MACNN':
    network = MACNNClassifier()
elif args.nn == 'TLENET':
    network = TLENETClassifier()
elif args.nn == 'FCN':
...
def start_server():
    if args.strategy == 'avg':
        strategy = customFedAvg(
            ...
            min_fit_clients=7,  # Minimum number of clients to be sampled for the next round
...
def create_client(cid, train_dataframe_user, test_dataframe_user):
    ...
    model = network.build_model(input_shape=(100,6), nb_classes=NUM_CLIENTS)
